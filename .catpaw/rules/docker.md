...
- 构建顺序必须先打包 workspace 依赖：先 `@aimo-console/logger`、再 `@aimo-console/dto`，最后 `@aimo-console/web` 和 `@aimo-console/server`。
...
- GitHub Actions 里要包含 `pnpm install`、`pnpm --filter @aimo-console/logger build`、`pnpm --filter @aimo-console/dto build`、`pnpm --filter @aimo-console/web build`、`pnpm --filter @aimo-console/server build`，再执行 `docker build`/`docker push`，确保构建产物和依赖完整。
